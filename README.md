...
矛盾ペナルティ:  confidence × Π(1 - 0.25 × severity)   // 未解決のうち重い上位3件のみ
```
...
**矛盾ペナルティを上位3件に制限**しているのは、矛盾が肯定×否定の総当たりで生成される＝証拠件数の
2乗で増えるからです。制限前は証拠60件で1要素あたり約101件の矛盾が生じ、`×0.75` が101回掛かって
Confidence が 0.0009 まで潰れていました（`validation/` E10）。なお方向の混在自体は事後分散が
すでに捉えている（割合が0.5付近＝分散最大）ので、ペナルティはその上の追加分です。
...
| 上記での0/100貼り付き | 36% | **0%** |
| 証拠を増やしたときのConfidence | 0.27 → **0.0009** | 0.24 → **0.31（単調増加）** |
...
5. **終了条件 `conf ≥ 0.75` は依然ほぼ到達しません。** 未解決の矛盾が3件以上ある要素は
   上限が `0.75³ ≒ 0.42` に張り付きます。加えて4の情報量の問題があるため、
