...
### 1.4 断定しない — 測定が支持する強さでしか主張しない

結果画面は「あなたは○○な人です」とは言いません。「今回の対話からは、○○の傾向が見えています」と表示し、Confidenceが低い軸には「情報不足」バッジを付け、レーダーチャートの塗りの濃さもConfidenceに連動させます。

さらに §13.0 の測定を受けて、**表示する量そのものを順序尺度に変えました**。

| 量 | 測定結果 | 表示 |
|---|---|---|
| 軸の**順位** | 真値との相関 r=0.98 / ICC 0.78 | ✅ 「10軸中 2位・相対的に高い」として表示 |
| 軸の**絶対値** | 真値の1/3.1に圧縮 | ❌ 0〜100の数値は出さない |
| 要素の**数値** | 再検査信頼性 r=0.29 | ❌ 3段階（現れている／どちらとも／あまり）と根拠件数のみ |

順位は「あなたの10軸を互いに比べた位置」であって、他人と比べた位置ではありません（比較のための基準集団を持っていないため）。結果画面にもそう明記しています。レーダーチャートの半径も順位で、目盛りは意図的に出していません——数値を振ればこの変更が避けようとした偽の精度が戻ってくるからです。
...
**この結論に合わせて表示を変えました**（§1.4）。軸は順位、要素は3段階＋根拠件数で、
0〜100の絶対値は画面から外してあります。`src/lib/engine/ordinal.ts` と
`tests/ordinal.test.ts` がその変換で、順位が「スコアの差の大きさ」ではなく
「順序」だけに依存することをテストで固定しています。
