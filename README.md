...
```
progress = 0.45×min(1, 成果ターン/15) + 0.35×min(1, conf/0.75) + 0.20×min(1, coverage/0.70)

終了 = (conf ≥ 0.75) かつ (coverage ≥ 0.70) かつ (未解決矛盾 ≤ 3)
  ただし最低10「成果ターン」は継続、30「総ターン」で強制終了
  直近3成果ターンのconf上昇が0.01未満なら飽和として終了可（→ 実装判断ログ #2）
```

**「成果ターン」= Evidenceを1件以上生んだターン**です（実装判断ログ #13）。
ターン番号自体は毎回進みます（会話ログが `(sessionId, turnIndex, role)` で
一意なため止められません）が、抽出が0件だったターンは progress にも最低ターン数にも
加算されません。強制終了の30だけは**総ターン**で数えます。抽出が失敗し続けても
インタビューが終わらなくなるのを防ぐためです。
...
| AI全断（Call A） | ターンを記録せず 503 `AI_UNAVAILABLE`・`turnCount` 据え置き・再送で復帰（判断ログ #12） |
| AI全断（Call B/Reply） | 会話継続（フォールバック質問）・クラッシュなし |
| 抽出0件 | 実質的な回答なら再抽出を1回実行。それでも0件なら継続するが、progress・最低ターン数・飽和判定のいずれにも加算しない（判断ログ #13） |
...
**#12 Call A の失敗はターンを中断する** ← §36 からの意図的な逸脱
仕様の§36は「抽出に失敗しても会話を止めない」と定めており、当初は失敗時に
Evidence 0件で会話を続けていました。しかしEvidence抽出は会話を診断に変える唯一の工程です。
失敗を握りつぶすと、**APIが全断していても対話だけが30ターン進み、`max_turns` で「完了」し、
中身が空のまま結果画面に到達します**（実測：30ターン完走・Evidence 0件・「完了」表示）。
ユーザーは30問に答えて、何も裏付けのない診断結果を受け取ることになります。

そこで Call A の失敗のみ例外を伝播させ、`AI_UNAVAILABLE` (503) を返してターンを記録しない
方針に変更しました。ユーザーの発話は先に保存済みで `turnCount` も据え置かれるため、
再送すれば同じターン番号で上書きされ、取りこぼしはありません。UI側も失敗時に入力内容を
テキストボックスへ戻すので、再送はワンタップです。

Call B（質問生成）とReplyの失敗は従来どおりフォールバックで継続します。これらは
「次に何を聞くか」の質の問題であり、フォールバック質問で十分に代替できるためです。
中断するのは**成果が残らないターン**だけに限定しています。

あわせて、抽出が0件だった場合（呼び出し自体は成功）も再抽出の対象に加えました。
従来は「ungrounded な引用が3件以上」のときだけ再実行していたため、
**空の抽出や1〜2件が全部棄却された場合は0件のまま黙って通過**していました
（`validation/quoteVerifier.ts`、テストあり）。20字未満の短い回答は
「読み取るものが無い」が妥当なので対象外です。

**#13 抽出0件のターンは進行に数えない**
#12 で「抽出の失敗」は止まるようになりましたが、**モデルが応答したうえで0件と答えた場合**は
依然として0件のままターンが記録されます（再抽出しても0件だった場合、および20字未満の回答）。

ここで「必ず1件出せ」とモデルに強制する案は採りませんでした。引用検証が確認できるのは
**その文字列を発言したかどうかだけで、その要素を支持するかは検証できません**。
出力を強制すると、10文字以上の断片を切り出して解釈を後付けした
「検証を通過する捏造証拠」が生まれます。空のモデルは目に見えて問題ですが、
捏造されたモデルは目に見えません（§1.3・§1.4）。再抽出プロンプトには
「判断材料が無ければ空配列を返してよい」と明示しています。

代わりに、**成果の無いターンを進行に数えない**方針にしました。

- `progress` のターン項は「Evidenceを生んだターン数」で計算する
- 最低10ターンの下限も成果ターンで数える（空のターンで下限に到達できない）
- 飽和判定の履歴からも空ターンを除外する。除外しないと、伸びていないのが
  「飽和したから」なのか「抽出できていないから」なのか区別できず、
  **何も学べていない状態で終了と判定されます**
- ただし**強制終了の30は総ターンで数える**。ここまで成果ターンにすると、
  抽出が壊れているセッションが永久に終わらなくなります

成果ターン数はDBに列を持たず `Evidence.turnId` の異なり数から導出します
（移行不要、値がずれない）。
...
- **同時実行はセッション単位のフラグで直列化しています。** `acquireSessionLock` は
  ...
  二重取得は起きません。
- **ロックの奪取は「リクエスト上限 < TTL」という前提に依存しています。** 停止した
  インスタンスが握ったままのロックを回収するため、`SESSION_LOCK_TTL_MS`(360秒)を過ぎた
  ロックは横取りできます。`releaseSessionLock` は所有者を確認せず解除するため、
  **まだ生きているリクエストからロックを奪えてしまうと二重処理が起こり得ます**。
  Vercelでは `maxDuration`(300秒) がその前に必ずリクエストを終了させ、さらに
  `TURN_AI_BUDGET_MS`(240秒) が先に自力で打ち切るため、この状況は発生しません
  （`tests/sessionLock.test.ts` が両方の関係を検証）。
  **リクエスト時間に上限のない環境へ移す場合は、ロックに所有者トークンを持たせて
  解除を条件付きにする必要があります。**
- **モデル呼び出しの期限は任意引数で受け渡しています。** `processTurn` が計算した
  ターン期限を各呼び出しへ渡していますが、型は `deadline?: number` です。新しい呼び出しを
  追加して渡し忘れても型エラーにはならず、その呼び出しだけが `CALL_BUDGET_MS`(150秒)
  単位の上限に戻ります（無制限にはなりません）。
