...
| 要素スコアと真値の相関 | r = 0.46 | r = 0.46（変わらず） |
| 再検査信頼性 | r = 0.23 | r = 0.23（変わらず） |
...
4. **1要素あたりの証拠が約2件しかありません**（30ターン・100要素）。
   この情報量では、更新式を正しくしても要素スコアは意味を持ちません
   （実測：30ターンでは新旧の式でRMSEがほぼ同じ）。要素スコアの r=0.46 と再検査信頼性 r=0.23 が
   変わらないのはこのためで、これは式ではなく**要素数と対話長の設計判断**の問題です。
