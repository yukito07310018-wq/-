...
デプロイ先で**ブラウザで開くだけ**で、対話が根拠を出せる状態かどうかと、
**直すための手順**が出ます。開発者でなくても操作できるように、ブラウザには
JSONではなくページを返します。

```
GET https://<your-app>/api/health              # ブラウザ → 手順ページ / スクリプト → JSON
GET https://<your-app>/api/health?format=json  # 常にJSON
GET https://<your-app>/api/health?probe=0      # 設定の確認のみ（上流を呼ばない）
```

ページの構成は「判定 → 各項目の合否 → **次にやること（番号付き手順）**」です。
手順には変更する画面の場所（`Settings → Environment Variables`）と、
**再デプロイしないと環境変数が反映されない**ことまで書いてあります。
これを書かないと「直したのに変わらない」で止まるためです。

複数落ちていても、手順は**1つだけ**出します。キーが無効なら接続確認も連動して落ちるので、
全部並べると先に直すべきでないものから手を付けてしまいます。

`turn_diagnostic_table` だけは対話を止めないので、**任意の改善**として区別して表示します
（破線の枠・「急ぎではありません」の注記付き）。緑の判定と並んで必須の手順のように出ると、
壊れていないものを壊れていると読ませてしまうためです。
...
同じ状態でも、伝える相手によって書くべきことが違います。

| 相手 | 見る場所 | 伝えること |
|---|---|---|
| 診断を受けた人 | 結果画面・会話画面 | **あなたの回答の問題ではない**こと。時間をおくか運営に伝える |
| 運営・開発者 | `/api/health`・ログ | どの環境変数が原因で、どこをどう直すか |

`describeExtraction()` が前者、`describeExtractionForOperator()` が後者です。
結果画面に「ANTHROPIC_API_KEY を確認してください」と出すのは、**直す手段を持たない人に
直し方を見せ、直せる人には何も見せていない**状態でした。しかも読んだ人には、自分の回答が
悪かったという意味に取れてしまいます。`extractionHealth.test.ts` で、利用者向けの文面に
環境変数名が混ざらないことを検証しています。

内訳の数値は結果画面にも残していますが、「運営向けの情報（そのままお伝えください）」と
明示しています。利用者が読むためではなく、スクリーンショットで渡してもらうためです。
