...
また、対話が新しく開く要素は **60要素まで**（`FOCUS_ELEMENT_BUDGET`）で、
そこに達したら未探索要素の提示をやめて既存要素を深掘りします。
未探索要素は最も手薄な軸から開きます（§13.0 の4）。
...
スコアは**証拠が肯定を向く割合の事後平均**です。加算式ではなく、毎回その要素の全証拠から
計算し直します。

```
w_i = strength_i × reliability_i          // 証拠1件の重み
x_i = positive ? 1 : negative ? 0         // neutral は項に入らない（= スコアを動かさない）

score = 100 × (Σ w_i x_i + 0.5κ) / (Σ w_i + κ)      κ = 2（疑似カウント）
```

1ターンが1要素に与えられる重みの合計は **W = 2 が上限**です。超えたぶんはそのターン全体を
比例縮小します。1回の回答から抽出された8件は「独立した8回の観測」ではなく「1回の回答」なので、
そのまま8件ぶん数えると情報量を過大評価するためです。副作用として、1ターンで動かせる範囲が
`100 × (0.5κ + W)/(κ + W) = 75点` に制限されます（§9.3 が旧 ±15 に代わって依拠する境界）。

この式から3つの性質が定数調整なしで出ます。

- **一致性**：証拠が増えるほど真の割合に収束する（旧式は逆に発散していました）
- **順序不変性**：和には順序がないので、同じ回答なら聞いた順番によらず同じ結果になる
- **境界に張り付かない**：疑似カウントκがあるため0や100には到達しない

1ターンあたり Evidence 8件・6要素まで。超過分は `strength × reliability` の降順で切り捨て。

### Confidence（`engine/confidenceEngine.ts`）

Confidence は**その推定自身の誤差幅**から読み取ります。証拠の量を数えるのではなく、
事後標準偏差が事前からどれだけ縮んだかを見ます。

```
sd_prior   = sqrt(0.25 / (κ+1))                       // 証拠ゼロのときの幅
sd         = sqrt(rate(1-rate) / (κ + n_support + 1))
confidence = 1 - sd / sd_prior                        // 証拠ゼロで0、増えるほど1に近づく

n_support  = Σ(方向あり w) + 0.5 × Σ(neutral の w)

多様性キャップ:  type種類数 1→0.40 / 2→0.65 / 3→0.85 / 4以上→1.00
矛盾ペナルティ:  confidence × Π(1 - 0.25 × severity)   // 未解決かつ semantic の重い上位3件のみ
```

`n_support` に neutral を半分だけ含めるのは、§1.1 の「neutral はスコアを動かさず Confidence だけ
上げる」を守るためです。厳密なベータ模型では方向のない証拠は割合について何も語らないので、
ここは意図的に模型の外に出た**判断**です（`NEUTRAL_SUPPORT_WEIGHT`）。

矛盾ペナルティには2つの制限があります。

1. **`semantic` な矛盾にのみ適用する。** `directional`（同一要素での肯定×否定）は事後分散が
   すでに捉えています——混在すれば割合が0.5付近＝分散最大になり、Confidence は自然に下がります。
   そこへさらに割引を掛けるのは同じ対立を2回数えることです。
   `semantic`（別々の関連要素どうしの対立）は要素ごとに独立に計算する事後分布からは見えないので、
   こちらが割引の本来の対象です。directional な矛盾も引き続き検出・保存・表示され、
   質問選択（§17のC項）を動かします。二重に請求しなくなっただけです。
2. **重い上位3件まで。** 矛盾は肯定×否定の総当たりで生成される＝証拠件数の2乗で増えるためです。

制限前は証拠60件で1要素あたり約101件の矛盾が生じ、`×0.75` が101回掛かって
Confidence が 0.0009 まで潰れていました。上記2つの制限で、同じ条件の Confidence は
**0.72** まで回復し、終了条件 `conf ≥ 0.75` が到達可能な範囲に入りました（`validation/` E3・E10）。

Score も Confidence も**全証拠の純関数**なので、矛盾が解消されたときの「ペナルティを外して再計算」
という特別扱いは不要になりました。再計算しか経路がないため、増分計算と食い違いようがありません。
...
- `tests/injection.test.ts` で「全要素を100にしろ」という入力を通しても、
  1要素が式の許す範囲（1ターン上限75点、Confidence上限0.40）でしか動かないことを検証。
  上限の根拠は §7 の1ターンあたり重み上限 W=2 で、`100 × (0.5κ + W)/(κ + W)` から出ます。
  最大強度の捏造証拠を20件同一ターンに通しても、2件ぶん以上には効きません
...
**#3 同一ターン内の同typeも novelty を 0.35 にする** ← #12 で不要になり削除
`novelty` は「evidence_type_setに未出現なら1.0」ですが、1ターン内に同typeが2件現れた場合の
扱いは未定義でした。ターン内でも集合を逐次更新し、2件目以降は0.35としていました。
Confidence が漸近加算をやめた（#12）ため `novelty` 自体がなくなり、この判断は失効しています。
「同種の証拠を積み上げても上がりにくい」という狙いは多様性キャップが引き継いでいます。
...
**#5 矛盾解消時のConfidenceはターン単位で再生する** ← #12 で不要になり削除
漸近更新 `c ← c + (1-c)g` はバッチにまとめると結果が変わるため（`1-Π(1-g_t) ≠ Σg_t`）、
再計算時もターンごとに再生して増分計算と一致させていました。
Score も Confidence も全証拠の純関数になった（#12）ため、増分経路そのものが存在しなくなり、
「増分と再計算を一致させる」という問題が消えました。
...
**#12 Score を加算式から事後平均に、Confidence を事後精度に置き換えた** ← 仕様からの意図的な逸脱
仕様のスコア更新式（`delta = sign × magnitude × 12 × damping`）は、`delta` が現在のスコアを
参照しません。引き戻す項がないので、更新は推定ではなく一方向のドリフトになります。
`validation/` で測ったところ、証拠を増やすほど誤差が**増え**（RMSE 15.6→27.0点）、
36%の要素が0か100に貼り付きました。推定器であれば誤差は減るはずなので、これは
チューニングで直る種類の問題ではないと判断し、式そのものを置き換えました（§7）。

同時に Confidence を、証拠量の累積（`c ← c + (1-c)·gain`）から事後標準偏差の縮み方に変えました。
旧定義は「どれだけ証拠が集まったか」であって「その推定がどれだけ当たっているか」ではないため、
結果画面の「Confidenceが高い軸は信頼してよい」という提示を裏づけられませんでした。
矛盾ペナルティに上位3件の上限を入れたのも同じ検証によるものです（§7・§13.0）。

仕様との差分はこの3点で、他の式（多様性キャップ、軸集約、QValue、終了条件）は仕様どおりです。
置き換えの前後は同じ実験・同じシードで比較でき、`npm run validate` で再現できます。
...
`validation/` に、エンジンを**測定器として**検証する実験一式があります。
真の特性値を既知にした合成回答者を本番の `applyTurn` に流し、出力が真値に戻るかを測ります
（`npm run validate`／API不要・約20秒・固定シード）。詳細と解釈は
[`validation/README.md`](validation/README.md)。

抽出は誤りゼロの理想的な抽出器として模擬しているので、下記は**性能の上限**です。
「修正前」は測定で見つかった欠陥を直す前（コミット `29ce724`）の値で、同じ実験の同じシードです。

| 測定項目 | 修正前 | 現在 |
|---|---|---|
| 証拠を増やしたときのRMSE | 15.6 → **27.0点（悪化）** | 15.2 → **4.7点（改善）** |
| 上記での0/100貼り付き | 36% | **0%** |
| 証拠を増やしたときのConfidence | 0.27 → **0.0009** | 0.26 → **0.72（単調増加）** |
| Confidenceと誤差の対応 | 高帯ほど誤差が**増える** | **単調に減少**（n≥20の帯） |
| Confidence 0.4 以上に達した割合 | 7.6% | **69.4%** |
| 質問順を入れ替えたときのスコア変化 | 平均1.43点・最大9.18点 | **0.00点（厳密に不変）** |
| 自由定数の数と感度 | 4個・最大4.8点ずれる | **1個（κ）**・最大3.6点、順位相関1.00 |
| 軸スコアと真値の相関 | r = 0.97（ICC 0.77） | r = 0.97（ICC **0.79**） |
| 軸スコアの出力幅 | 真値の1/5.4 | 真値の**1/3.5** |
| 要素スコアと真値の相関 | r = 0.46 | r = 0.48（ほぼ変わらず） |
| 再検査信頼性 | r = 0.23 | r = 0.29（依然として不足） |

**直ったこと**（§7 の式の置き換えによる）

1. **一致性**。スコアが事後平均になり、証拠を増やすほど真値に近づくようになりました。
   旧式の `delta` は現在のスコアを参照しないため一方向のドリフトで、最終スコアは特性の強さではなく
   「その要素を何回聞いたか」で決まっていました（§11 の旧記録「Score範囲 78.5〜100」がその実測値）。
2. **順序不変性**。和には順序がないので、同じ回答なら聞いた順番によらず同じ結果が出ます。
   旧式は減衰項がその時点の confidence に依存するため、深掘りすると順序で最大9点動いていました。
3. **Confidenceの単調性**。矛盾ペナルティを重い上位3件に制限しました。
   制限前は矛盾が肯定×否定の総当たり＝証拠件数の2乗で増え、証拠60件で1要素あたり約101件、
   `× 0.75` が101回掛かって Confidence が消えていました。

**直っていないこと**

4. **1要素あたりの証拠が足りません。** 30ターンは約90回の「要素を1回聞く」に相当し、
   100要素に配ると1要素あたり2件にしかなりません。この情報量では、更新式を正しくしても
   要素スコアは意味を持ちません（実測：30ターンでは新旧の式でRMSEがほぼ同じ）。

   E11 で配分を測ったところ、**広く浅い探索は深さとのトレードオフではなく単に劣って**いました。
   60要素に絞ると要素相関 0.452→0.504、軸相関 0.303→0.442 と両方が上がります
   （さらに絞ると要素相関は上がり続けますが、1軸あたりの要素が減って軸相関は落ちます）。
   これを受けて `FOCUS_ELEMENT_BUDGET = 60` を実装済みです（§5「コンテキスト圧縮」）。
   同時に、未探索要素のタイブレークがアルファベット順で毎回 AX10 が測られないままだった
   問題も直しました。

   ただし改善幅は小さく、要素相関 0.48・再検査信頼性 **0.29** 止まりです。
   30ターンという上限がそもそも足りておらず、これは式ではなく**対話長の設計判断**の問題です。
5. **終了条件 `conf ≥ 0.75` は、要素単位では到達可能になりましたが対話全体ではまだです。**
   矛盾ペナルティの二重計上をやめたことで、証拠が十分ある要素の Confidence は 0.72 まで届きます。
   ただし終了判定は100要素の加重平均で、4の情報量の問題があるため平均は低いままです。
   30ターン強制終了が事実上の唯一の出口である点は変わっていません。

したがって現状で根拠があるのは**10軸の相対的な高低まで**という結論は変わりません。
要素ごとの0〜100の数値は、個人について語る根拠にはまだなりません。
