...
| 測定項目 | 結果 |
|---|---|
| 軸スコアと真値の相関 | **r = 0.97**（ICC(1) = 0.77）— 軸レベルの順序は妥当 |
| 要素スコアと真値の相関 | r = 0.46（分散の21%）。「常に50」に対しRMSEで2点しか勝たない |
| 再検査信頼性 | **r = 0.23** — 同一人物の2回で要素あたり平均8.8点ずれる |
| 証拠を増やしたときのRMSE | 15.6点 → **27.0点（悪化）**。36%の要素が0か100に貼り付く |
| 証拠を増やしたときのConfidence | 0.27 → **0.0009**（矛盾ペナルティの累乗で潰れる） |
| 軸スコアの出力幅 | 真値の**1/5.4**に圧縮（10軸すべて44〜56点に収まる） |

導かれる帰結は3つです。

1. **スコア更新式に引き戻し項がありません。** `delta` は現在のスコアを参照しないため、
   更新は推定ではなく一方向のドリフトです。最終スコアは特性の強さではなく
   「その要素を何回聞いたか」で決まります（§11 の「Score範囲 78.5〜100」はこの性質の実測値）。
   ただし順位相関は0.91を保つので、**壊れているのは目盛りであって並び順ではありません。**
2. **矛盾は証拠件数の2乗で増えます。** 肯定×否定の総当たりでペアを作るため、
   証拠60件で1要素あたり約101件の未解決矛盾が生じ、
   `× (1 − 0.25·severity)` がその回数だけ掛かってConfidenceが消えます。
   結果として終了条件 `conf ≥ 0.75` は対話を延ばすほど遠のき、30ターン強制終了が事実上の唯一の出口です。
3. **1要素あたりの証拠が約2件しかありません**（30ターン・100要素）。
   この情報量では、更新式を正しいものに差し替えても要素スコアは意味を持ちません
   （検証済み：同一証拠で事後平均式に替えても30ターンでは改善しない）。

したがって現状で根拠があるのは**10軸の相対的な高低まで**で、
要素ごとの0〜100の数値と Confidence の数値は、個人について語る根拠になりません。
